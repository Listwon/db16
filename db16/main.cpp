...
int main(int argc, char *argv[]) {
	char* filename = argc > 1 ? argv[1] : "db16.png";
	...
	std::string output = "db16_" + fullname.substr(0, fullname.find_last_of(".")) + ".png";
	...
	for (unsigned y = 0; y < img.h; y++) {
		for (unsigned x = 0; x < img.w; x++) {
			...
			/*
			for (int c = 0; c < 16; c++) {
			diff16[c] = (db16[c * 3 + RGBA::R] - img.data[img.format * (img.w * y + x) + RGBA::R])*(db16[c * 3 + RGBA::R] - img.data[img.format * (img.w * y + x) + RGBA::R]) +
			(db16[c * 3 + RGBA::G] - img.data[img.format * (img.w * y + x) + RGBA::G])*(db16[c * 3 + RGBA::G] - img.data[img.format * (img.w * y + x) + RGBA::G]) +
			(db16[c * 3 + RGBA::B] - img.data[img.format * (img.w * y + x) + RGBA::B])*(db16[c * 3 + RGBA::B] - img.data[img.format * (img.w * y + x) + RGBA::B]);
			if (diff16[c] < min) {
			min = diff16[c];
			cindex = c;
			}
